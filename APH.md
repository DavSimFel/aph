...
**Upstream's three Linux enterprise jobs cannot run here.** `ci.yml` pins its static, coverage, and snapshot jobs to the `dsh-ubuntu-24-04-16core` pool and its native Windows job to `dsh-windows-2025-16core`. Those labels belong to the upstream organization; this repository has no runner carrying them, so the jobs queue until they expire, reporting neither success nor failure. [aph-ci.yml](.github/workflows/aph-ci.yml) runs `check:ci:static` on a GitHub-hosted runner instead. It is a new file rather than an edit to `ci.yml` because upstream owns that file and every edit to it returns as a conflict on each sync.

Hosted runners are smaller than the pools upstream's fixtures were recorded on and carry different tooling, and two tests depend on that difference, so the lane runs the static suite alone. Coverage and snapshots are absent rather than advisory: a suite that reports a failure nobody can act on puts a permanent red on every pull request and teaches the reader to ignore red. The workflow's own comment records which tests fail and why, and [#14](https://github.com/DavSimFel/aph/issues/14) restores both as blocking gates.

The lane's jobs are named `aph / <suite>`. `dev` is protected and requires `aph / static`, which is why the name may not repeat one of upstream's: a required check matches on job name, and `ci.yml` publishes `node 24 / static` from a job that never gets a runner here, so a shared name would leave every merge waiting on the one that cannot report.
