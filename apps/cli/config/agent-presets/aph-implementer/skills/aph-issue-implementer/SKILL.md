...
- runs `pnpm install --frozen-lockfile --prefer-offline` in the worktree so each session owns its writable `node_modules` links while all issue worktrees reuse the workspace-local `.aph-worktrees/.pnpm-store` content-addressed store;
...
Fill a temporary copy of `aph/templates/pr.md`; nothing precedes **For the operator**. Include the issue link and Intent, behavior delivered, an exact demonstration command or URL, every decision beyond the issue or `none`, and risk with rollback. Keep implementation details and exact check commands in the collapsed section.

Run `handoff` with that body file and title. It finds the branch's existing open PR before creating one, requires an open draft from the issue branch to `dev`, recovers when creation succeeded but its response was lost, idempotently links the PR on the issue, and moves `stage/in-session` to `stage/agent-review`. A rerun after either the PR creation or stage move continues without duplicating the PR or comments.

Apply one `kind/*` label and every material `area/*` label. For GUI-visible changes, load `record-browser-gif`, record the exact committed PR head from a clean branch tree with a real server and model round, publish the GIF on the append-only assets branch, and embed it with commit, tree, origin, mode, browser-state, and real-model provenance. Never substitute fixtures or a no-model recording.
